...
sed -i "s/^Server=/&${agentd_ip},/" /usr/local/etc/zabbix_agentd.conf
sed -i "s/^ServerActive=/&${agentd_ip_port},/" /usr/local/etc/zabbix_agentd.conf
sed -i "s/^Hostname=/&${agentd_hostname}/" /usr/local/etc/zabbix_agentd.conf
